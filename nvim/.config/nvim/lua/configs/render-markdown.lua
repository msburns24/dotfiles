...
return {
    ...
    -- :h mode(), for all enabled components. Individual components can be
    -- enabled for other modes. Remaining modes will be unaffected by this
    -- plugin.
    render_modes = { 'n', 'c', 't' },


    -- Maximum file size (in MB) that this plugin will attempt to render. Any
    -- file larger than this will effectively be ignored.
    max_file_size = 10.0,


    -- Milliseconds that must pass before updating marks, updates occur within
    -- the context of the visible window, not the entire buffer.
    -- debounce = 100,
    debounce = 300,


    -- Pre configured settings that will attempt to mimic various target user
    -- experiences. Any user provided settings will take precedence.
    --
    ...
    -- Print runtime of main update method. Only intended to be used for
    ...
    -- Takes buffer as input, if it returns true this plugin will not attach
    -- to the buffer
    -- ignore = function()
        -- return false
    -- end,
    ...
    -- Out of the box language injections for known filetypes that allow
    -- markdown to be interpreted in specified locations. See
    -- :h treesitter-language-injections. Set enabled to false in order to
    -- disable.
    -- injections = {
        -- gitcommit = {
            -- enabled = true,
    ...
    -- Highlight patterns to disable for filetypes, i.e. lines concealed
    -- around code blocks
    patterns = {
        markdown = {
            disable = true,
    ...
    heading = {
        ...
        -- Always use virtual lines for heading borders instead of attempting
        -- to use empty lines.
